...
class ChatUserRoom extends StatefulWidget {
  ...
  String receiverName;
   ChatUserRoom({ Key? key,
     required this.receiverName,required this.receiverimageLink,required this.receiverId,
   }) : super(key: key);
...
TextEditingController messageController=TextEditingController();
  ...
  void initState() {
    ...
    if (FirebaseAuth.instance.currentUser!.uid.hashCode <= widget.receiverId.hashCode) {
      groupChatId = "${FirebaseAuth.instance.currentUser!.uid}-${widget.receiverId}";
    }  else {
      groupChatId = "${widget.receiverId}-${FirebaseAuth.instance.currentUser!.uid}";
    }
    ...
    return Scaffold(
              ...
              children: <Widget>[
                ...
                Expanded(
                  child: Column(
                    ...
                    children: <Widget>[
                      Text(widget.receiverName,style: TextStyle( fontSize: 16 ,fontWeight: FontWeight.w600),),
                      SizedBox(height: 6,),
                      Text(myStatus,style: TextStyle(color: Colors.grey.shade600, fontSize: 13),),
                    ],
                  ),
                ),
              ],
      ...
      body: Container(
        ...
        children: <Widget>[
          ...
          StreamBuilder(
                  stream:  FirebaseFirestore.instance.collection("messages").doc(groupChatId).collection(groupChatId).orderBy("timestamp",descending: false).snapshots(),
                  builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
                    if (snapshot.hasData) {

                      return snapshot.data!.docs==0?Center(child: Text("Empty ")): ListView.builder(
                        ...
                        itemBuilder: (context, index){
                          ...
                          Container(
                            ...
                            child: Align(
                              alignment: (ds.get("senderId") == FirebaseAuth.instance.currentUser!.uid?Alignment.bottomRight:Alignment.bottomLeft),
                              child: Container(
                                ...
                                child: Text(ds.get("content"), style: TextStyle(fontSize: 15),),
                              ),
                            ),
                          ):
                          ds.get("type")==1?Container(
                            ...
                            child: Align(
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  borderRadius: BorderRadius.circular(20),
                                  image: DecorationImage(image: NetworkImage(ds.get("image"),),fit: BoxFit.fill),
